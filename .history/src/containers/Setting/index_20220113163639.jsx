...
export default function Setting() {
    ...
    return (<div className={`relative transform duration-500 ${active ? "translate-x-0" : "translate-x-40"}`}>
        <button className="btn-shadow dark:btn-shadow-dark-50 setting -left-56 top-8 rounded-full text-center w-10 h-10" onClick={() => setActive(!active)}>
            <FaRegSun class="animate-spin duration-1000 text-3xl" style={{ animationDuration: "2s" }} fill={colors.coolGray[400]} />
        </button>
        <div className={`absolute top-0 right-0 rounded-3xl flex justify-between p-7 shadow-3xl `} style={{ backgroundColor: "#ecf0f3" }} >
