...
export default function Setting() {
    ...
    return (<div className={`relative transform duration-500 ${active ? "translate-x-0" : "translate-x-40"}`}>
        <button className="btn-shadow dark:btn-shadow-dark bg-app dark:bg-neutral-800 setting -left-56 top-8 rounded-full text-center w-10 h-10" onClick={() => setActive(!active)}>
        ...
        <div className={`absolute top-0 right-0 rounded-3xl flex justify-between p-7 shadow-3xl bg-app dark:bg-neutral-800 dark:shadow-4xl`} >
            ...
            <button className="mr-2 btn-shadow dark:btn-shadow-dark btn-setting rounded-full text-center w-12 h-12" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
                {theme === 'dark' ? (
                    <FaSun
